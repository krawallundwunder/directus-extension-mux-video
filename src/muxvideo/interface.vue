<template>
    <div class="mux-video">
        <mux-uploader
            ...
            id="uploader"
        >
            <v-button slot="file-select" secondary rounded icon v-tooltip="$t('click_to_browse')">
        ...
        <div v-else-if="status === 'ready'" class="player-container">
            ...
            />
            <v-remove @action="handle_remove" class="remove-button" button confirm secondary>
                Remove Video
            </v-remove>
        </div>

        <div v-else-if="status === 'errored'" class="message-container">
            <v-icon name="exclamation-triangle" class="icon" x-large />
        ...
        >
            <v-icon name="refresh" class="spin icon" x-large />
...
async function create_upload_url() {
    try {
        ...
        upload_url_expires_at.value =
            new Date().getTime() + response.data.timeout;
...
async function update_video_record() {
    ...
    try {
        const response = await api.patch(
            `/items/${mux_uploads_collection}/${props.value}`,
            {
                upload_id: upload_id.value,
                status: status.value,
